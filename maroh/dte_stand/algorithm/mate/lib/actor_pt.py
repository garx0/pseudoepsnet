...
from dte_stand.algorithm.mate.config import ActorCfg, SaActorCfg
...
class Actor(torch.nn.Module):
    def __init__(self,
        ...
        self.use_state_before_init = True

    def init_memory(self):
        # TODO: GAT
        kwargs = {}
        if self.cfg.memory_type == 'epsilon_net':
            Memory = EpsilonNetMemory
            kwargs.update(representatives_path=self.cfg.representatives_path)
        elif self.cfg.memory_type == 'knn_classifier':
            Memory = KNNClassifierMemory
            kwargs.update(first_iter_active=self.cfg.first_iter_active, all_agents=self.cfg.all_agents,
                representatives_path=self.cfg.representatives_path)
        elif self.cfg.memory_type == 'centroids':
            Memory = CentroidsMemory
            kwargs.update(first_iter_active=self.cfg.first_iter_active, all_agents=self.cfg.all_agents,
                representatives_path=self.cfg.representatives_path)
        else:
            Memory = EpsilonNetMemory
    ...
    def local_aggregation(self, link_states, first_iter=False): # aggregate local states
        # TODO
        if first_iter == True and self.use_state_before_init:
    ...
    def message_passing(self, input_tensor): # input_tensor - vector of length n_links*2
        ...
        # padding
        link_states_init = torch.reshape(input_tensor, (self.num_features, self.n_links))
        link_states_init = link_states_init.T
        ...
        # print(f"message_iteration=0, link_states before init: {link_states}")
        link_states = self.link_state_initializer(link_states_init)
        if self.states_logging and not self.training and not self.eval_mode:
            if self.use_state_before_init:
                self.log_states[0].append(link_states_init.detach().cpu().numpy())
            else:
                self.log_states[0].append(link_states.detach().cpu().numpy())
        # print(f"message_iteration=0, link_states after init: {link_states}")
        # link_states - tensor of shape (n_links, 16)
        codes = []
        last_states = {}
        final_actions = {}
        iterations_done = 0
        iterations_done_dict = {i: self.message_iterations for i in range(self.n_links)}
        # message iterations
        for message_iteration in range(self.message_iterations):
            ...
            if self.memory_used or self.mode == '4' or self.mode == '5':
                if not self.use_state_before_init or not self.memory_used or message_iteration > 0:
                    ...
                    memory_states = link_states_init
                # print(f"{message_iteration=}, memory_states: {link_states}")
                # print(f"{message_iteration=}, {link_states.shape=}, {memory_states.shape=}")
                # memory states - either 2, 6, 16, or 48 long vectors
            if self.memory_used and not self.training:
                memory_states_pad = memory_states.detach().cpu().numpy().astype(float)
                if self.use_state_before_init and message_iteration == 0 and type(self.mind[0]) == EpsilonNetMemory:
                    memory_states_pad = np.pad(memory_states_pad,
                        [(0, 0), (0, self.link_state_size - self.num_features)])
                code, act_memory = self.mind[message_iteration].update_memory(memory_states_pad, update=self.update_memory)
                codes.append(code)
                for i, c in enumerate(code):
                    if i not in final_actions and c in (ACT_VEC_USE, ACT_VEC_USE_AND_UPDATE):
                        last_states[i] = link_states[i]
                        final_actions[i] = act_memory[i]
                for i in final_actions:
                    iterations_done_dict[i] -= 1
                if len(final_actions) == self.n_links:
                    if self.states_logging and not self.training and not self.eval_mode:
                        for i in range(message_iteration, self.message_iterations):
                            self.log_states[i + 1].append(link_states.detach().cpu().numpy())
                    for i in range(self.n_links):
                        iterations_done_dict[i] -= (self.message_iterations - message_iteration - 1)
                    return codes, link_states, final_actions, iterations_done, iterations_done_dict
            ...
            for i, state in last_states.items():
                link_states[i] = state
            if self.states_logging and not self.training and not self.eval_mode:
            ...
            if self.mode == '2' or self.mode == '3' or self.mode == '5':
                ...
                for i, state in last_states.items():
                    link_states[i] = state
            iterations_done += self.n_links - len(final_actions)
        return codes, link_states, final_actions, iterations_done, iterations_done_dict
    ...
    def forward(self, input_tensor):
        codes, link_states_t, memory_actions, iterations_done, iterations_done_dict = self.message_passing(input_tensor)
        if self.memory_used and not self.training:
            self.memory_used = False
            states_logging = self.states_logging
            self.states_logging = False
            _, link_states_t_true, _, _, _ = self.message_passing(input_tensor)
            self.memory_used = True
            self.states_logging = states_logging

        if self.memory_used and not self.training:
            non_mem_acts_idx = torch.ones(self.n_links, device=self.device, dtype=torch.bool)
            for i in memory_actions.keys():
                 non_mem_acts_idx[i] = False
            policy = torch.zeros(self.n_links, self.num_actions, device=self.device, dtype=torch.float32)
            policy[non_mem_acts_idx] = self.readout(link_states_t[non_mem_acts_idx])
            policy_true = torch.zeros(self.n_links, self.num_actions, device=self.device, dtype=torch.float32)
            policy_true[non_mem_acts_idx] = self.readout(link_states_t_true[non_mem_acts_idx])
            policy_true[~non_mem_acts_idx] = self.readout(link_states_t_true[~non_mem_acts_idx])
            # policy_true2 = self.readout(link_states_t_true)
            # assert torch.mean(torch.abs(policy_true2 - policy_true)) < 1e-3, f"{torch.abs(policy_true2 - policy_true)}"
            # policy_true[:] = policy
            # policy_true[~non_mem_acts_idx] = self.readout(link_states_t[~non_mem_acts_idx])
            # policy_true = self.readout(link_states_t)
            # policy[:] = policy_true
            for i, act in memory_actions.items():
                policy[i, act] = 1.
            self.log_message_iterations_done += iterations_done
            self.log_message_iterations_possible += self.message_iterations * self.n_links
            if not self.eval_mode:
                self.log_message_iterations_done_agents.append(np.array([iterations_done_dict[i] for i in range(self.n_links)]))
        else:
            policy = self.readout(link_states_t)
            policy_true = None
        policy = torch.reshape(policy, (-1,))
        if policy_true is not None:
